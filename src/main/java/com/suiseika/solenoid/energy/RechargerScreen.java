...
/**
 * Fully code-drawn Recharger GUI, following the {@link InductionFurnaceScreen} pattern: panel, slot
 * recesses, a vertical EMF bar for the block buffer, and a charge arrow that lights while energy is
 * flowing into the slotted item. All painted with {@link GuiGraphicsExtractor#fill} (no texture sheet).
 */
public class RechargerScreen extends AbstractContainerScreen<RechargerMenu> {
    ...
    private static final int ARROW_X = 48, ARROW_Y = 34, ARROW_W = 24, ARROW_H = 16;
    private static final int ARROW_LIVE = 0xFF55E03A;
    ...
    public void extractBackground(GuiGraphicsExtractor g, int mouseX, int mouseY, float a) {
        ...
        // Charge arrow (lights when energy is available and an item sits in the slot)
        int ax = x + ARROW_X, ay = y + ARROW_Y;
        g.fill(ax, ay, ax + ARROW_W, ay + ARROW_H, PANEL_DARK);
        boolean charging = stored > 0 && this.menu.slots.get(0).hasItem();
        if (charging) {
            g.fill(ax, ay, ax + ARROW_W, ay + ARROW_H, ARROW_LIVE);
        }
