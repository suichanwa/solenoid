...
/**
 * Digester: cracks the concentrate with a lye reagent. Slot 0 input, slot 1 reagent, slots 2-4
 * outputs. Consumes 1 input + 1 reagent per completed op.
 */
public class DigesterBlockEntity extends AbstractEmfBlockEntity implements MenuProvider {
    private static final int INPUT = 0;
    private static final int REAGENT = 1;
    private static final int OUT_FIRST = 2;
    private static final int OUT_LAST = 4;

    private static class DigesterItemHandler extends ItemStacksResourceHandler {
        private final DigesterBlockEntity be;

        DigesterItemHandler(DigesterBlockEntity be) {
            super(5);
            this.be = be;
        }
    ...
    private final DigesterItemHandler itemHandler = new DigesterItemHandler(this);
    ...
    public Component getDisplayName() {
        return Component.translatable("container.solenoid.digester");
    }
    ...
    protected void serverTick(net.minecraft.server.level.ServerLevel level, BlockPos pos, BlockState state) {
        ...
        Optional<RecipeHolder<DigestingRecipe>> recipeOptional =
                level.recipeAccess().getRecipeFor(SolenoidRecipes.DIGESTING_TYPE.get(), recipeInput, level);
        ...
        DigestingRecipe recipe = recipeOptional.get().value();
        if (!recipe.reagent().test(reagent) || !canProcess(recipe)) {
            setWorking(false);
            resetProgress();
            return;
        }
    ...
    /** True if the first (guaranteed) output has room. */
    ...
    private void processRecipe(net.minecraft.server.level.ServerLevel level, DigestingRecipe recipe) {
        ItemStack input = itemHandler.getStack(INPUT);
        input.shrink(1);
        ...
        ItemStack reagent = itemHandler.getStack(REAGENT);
        reagent.shrink(1);
        itemHandler.setStack(REAGENT, reagent);
